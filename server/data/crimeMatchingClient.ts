...
export default class CrimeMatchingClient extends RestClient {
  ...
  getCrimeVersion(authOptions: AuthOptions, crimeVersionId: string): Promise<unknown> {
    return this.get(
      {
        path: `/crime-versions/${crimeVersionId}`,
      },
      authOptions,
    )
  }
  ...
  getIngestionAttempt(authOptions: AuthOptions, ingestionAttemptId: string): Promise<unknown> {
    return this.get(
      {
